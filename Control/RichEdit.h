...
public:
	...
	typedef std::function<bool(CHARRANGE crLink)> FunCustomLinkMouseEnter;
public:
	RichEdit();
	...
	void SetLimitMaxLines(int32_t nLimitMaxLines);
	/**
	 * @brief 恢复操作
	 * @return 成功返回 true，失败返回 false
	 */
	...
	ITextServices2* GetTextServices2();
	BOOL SetOleCallback(IRichEditOleCallback* pCallback);
	CSize GetNaturalSize(LONG width, LONG height, bool bNeedDpiScale = true);
	//获取富文本高宽
	CSize GetRichTextNaturalSize(LONG width, LONG height, bool bNeedDpiScale = true);
	...
	void SetRichEditTextWithLimit( int nMaxLines);
	...
	/**
	* @brief 设置一个带有文字颜色的超链接
	* @param[in] nStartChar link起始位置
	* @param[in] nEndChar link结束位置
	* @param[in] linkInfo 链接地址
	* @return 无
	*/
	void SetLinkColorBySel(long nStartChar, long nEndChar, const std::wstring &color, const std::wstring &linkInfo = L"");
	 ...
	 */
	bool HittestCustomLink(CPoint pt, std::wstring& info, CHARRANGE& cr);
	...
	/**
	* @brief 监听自定义链接鼠标移入事件
	* @param[in] callback 自定义链接鼠标移入后的自定义回调函数
	* @return 无
	*/
	void AttachCustomLinkMouseEnter(const FunCustomLinkMouseEnter& callback)	{ m_cbCustomLinkMouseEnter = callback; }

	/**
	* @brief 设置富文本使用d2d绘制
	* @return 无
	*/
	static void SetUseDirect2d() { m_useD2d = true; }

	/**
	* @brief 富文本是否使用D2D绘制
	* @return 无
	*/
	static bool IsUseDirect2d() { return m_useD2d; }
private:
	void ImeUpdateCompositionPos();
...
protected:
	...
	int  m_iLimitText;
	LONG m_lTwhStyle;
	double m_nLineSpacing = 0.0;
	int32_t m_nLimitMaxLines = 0; //大于0 才开启限制
	UiRect m_rcPrompttextPadding;
	LONG m_lPrompttextStyle;
	...
	FunCustomLinkMouseEnter m_cbCustomLinkMouseEnter;
	static bool m_useD2d;
protected:
	struct LinkInfo
	{
		CHARRANGE cr;
...
private:
	nbase::WeakCallbackFlag m_imeUpdatePosFlag;
};

//判断是否是字节： 可打印字符（0x20-0x7e）
UILIB_API bool IsAsciiChar(const wchar_t ch);
//获取字符串的字节数
UILIB_API int  GetAsciiCharNumber(const std::wstring &str);
//删除字符串中超过limit字节个数之后的字符
UILIB_API void LimitAsciiNumber(std::wstring &src, int limit);
//获取粘贴板字符串
UILIB_API void GetClipboardText(std::wstring &out);
//设置粘贴板字符串
UILIB_API void SetClipBoardText(const std::wstring &str);
