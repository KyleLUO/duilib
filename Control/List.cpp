...
bool ListBox::SelectItem(int iIndex, bool bTakeFocus, bool bTrigger)
{
	...
	if (GetItemAt(m_iCurSel) && bTrigger) {
	...
	for (auto it = m_items.begin(); it != m_items.end(); it++) {
		if ((*it)->GetName() == strItemName) 
		{
			if (GetScrollRange().cy != 0) 
			{
				...
				int internal_top = m_pLayout->GetInternalPos().top;
				scrollPos.cy = (*it)->GetPos().top - internal_top;
				if (scrollPos.cy >= 0) {
...
bool ListBox::ScrollItemToTop(int index)
{
	ui::Control *find_control = nullptr;
	if (m_items.size() > index && index != -1)
	{
		find_control = m_items.at(index);
	}
	
	if (find_control && GetScrollRange().cy != 0)
	{
		CSize scrollPos = GetScrollPos();
		int internal_top = m_pLayout->GetInternalPos().top;
		scrollPos.cy = find_control->GetPos().top - internal_top;
		if (scrollPos.cy >= 0) {
			SetScrollPos(scrollPos);
			return true;
		}
...
bool ListBox::ScrollItemToBottom(const std::wstring& strItemName)
{
	for (auto it = m_items.begin(); it != m_items.end(); it++) 
	{
		if ((*it)->GetName() == strItemName) 
		{
			if (GetScrollRange().cy != 0)
			{
				...
				int internal_top	= m_pLayout->GetInternalPos().top;
				int internal_bottom = m_pLayout->GetInternalPos().bottom; 

				scrollPos.cy = (*it)->GetPos().bottom - internal_bottom;
				if (scrollPos.cy >= 0) {
					SetScrollPos(scrollPos);
					return true;
				}
...
void ListBox::Previous(bool bTrigger/* = true*/)
{
	if (m_iCurSel > 0) {
		SelectItem(m_iCurSel - 1, false, bTrigger);
	}
}

void ListBox::Next(bool bTrigger/* = true*/)
{
	int count = GetCount();
...
bool ListBox::RemoveAt(int iIndex)
{
	...
	if( iIndex == m_iCurSel && m_iCurSel >= 0 ) {
		if (m_bSelNextWhenRemoveActive && m_iCurSel< GetCount())
...
void ListContainerElement::Selected(bool bSelected, bool trigger)
{
	...
	{
		m_pOwner->SelectItem(-1, false, trigger);
	}
}
