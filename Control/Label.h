...
public:
	...
	virtual void SetHighlightText(const std::wstring& strText, const std::wstring& strColor);
	 ...
	 * @return 无
	 */
	virtual void SetStateTextColor(ControlStateType stateType, const std::wstring& dwTextColor);
	 ...
	 * @return 无
	 */
	virtual void SetFont(const std::wstring& strFontId);
	 ...
	 * @return 无
	 */
	virtual void SetTextPadding(UiRect rc);
	...
	virtual void SetShowHtml(bool bShowHtml);

	void SetTextData(const std::wstring& strTextData);
protected:
	...
	std::wstring	m_sHighlightText;
	std::wstring	m_sHighlightColor;
	std::wstring    m_strTextData;
};
...
template<typename InheritType>
void LabelTemplate<InheritType>::SetHighlightText(const std::wstring& strText, const std::wstring& strColor)
{
	m_sHighlightText = strText;
	m_sHighlightColor = strColor;
}
...
/*	else
	{
		int dpi_scale = ui::DpiManager::GetInstance()->GetScale();
		width = width * 100 / dpi_scale;
	}*/

	std::wstring  text_data = GetText();
	if (m_bShowHtml && !m_strTextData.empty())
	{
		text_data = m_strTextData;
	}
	CSize fixedSize;
	if (!text_data.empty()) {
		...
		UiRect rect = pRender->MeasureText(text_data, m_sFontId, m_uTextStyle, width);
		if (this->GetFixedWidth() == DUI_LENGTH_AUTO) {
			fixedSize.cx = rect.right - rect.left + m_rcTextPadding.left + m_rcTextPadding.right + 3;
		}
		if (this->GetFixedHeight() == DUI_LENGTH_AUTO) {
			...
			fixedSize.cx = estimateWidth + m_rcTextPadding.left + m_rcTextPadding.right + 3;
...
void LabelTemplate<InheritType>::SetAttribute(const std::wstring& strName, const std::wstring& strValue)
{
	...
	else if (strName == _T("showhtml"))SetShowHtml(strValue == _T("true"));
...
void LabelTemplate<InheritType>::PaintText(IRenderContext* pRender)
{
	...
	{
		RECT prcLinks;
		pRender->DrawHtmlText(rc, GetText(), dwClrColor, m_sFontId, m_uTextStyle, &prcLinks);
		return;
	}

	if (m_sHighlightText.size() > 0)
	{
		DWORD dwHighlightColor = GlobalManager::GetTextColor(m_sHighlightColor);
		pRender->DrawHighlightText(rc, GetText(), dwClrColor, m_sHighlightText, dwHighlightColor, m_sFontId, m_uTextStyle, 255, m_bLineLimit);
		return;
	}
	pRender->DrawText(rc, GetText(), dwClrColor, m_sFontId, m_uTextStyle, 255, m_bLineLimit);
}
...
template<typename InheritType>
void LabelTemplate<InheritType>::SetShowHtml(bool bShowHtml)
{
...
template<typename InheritType>
void LabelTemplate<InheritType>::SetTextData(const std::wstring& strTextData)
{
	m_strTextData = strTextData;
}
