...
public:
	...
	bool ScrollItemToTop(int index);
	/**
	* @brief 滚动到指定子项位置
	* @param[in] strItemName 子项名称
	* @return 成功返回 true，否则为 false，可能控件不存在
	*/
	virtual bool ScrollItemToBottom(const std::wstring& strItemName);
	 ...
	 * @return 无
	 */
	void Previous(bool bTrigger = true);

	/**
	 * @brief 选中下一项
	 * @return 无
	 */
	void Next(bool bTrigger = true);
