...
ui::TreeView* TreeNode::GetTreeView()
{
	return m_pTreeView;
}
...
bool TreeNode::AddChildNodeAt(TreeNode* pTreeNode, std::size_t iIndex)
{
	...
	if (m_iDepth != ROOT_NODE_DEPTH) {
		nodeIndex = GetIndex();
		padding.left += (m_pTreeView->GetIndent() + this->GetIndent());
	}
	pTreeNode->m_pLayout->SetPadding(padding,false);
...
ui::TreeNode* TreeNode::FindChildNode(const std::wstring &node_name)
{
	ui::TreeNode *find_node = nullptr;
	ui::TreeNode *cur_node = nullptr;

	for (int index = 0; index < this->GetChildNodeCount(); index++)
	{
		cur_node = GetChildNode(index);
		if (cur_node->GetName() == node_name)
		{
			find_node = cur_node;
			break;
		}
...
int TreeNode::GetIndent()
{
	return m_iIndent;
}

void TreeNode::SetIndent(int indent)
{
	m_iIndent = indent;
}
