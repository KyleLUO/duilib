...
bool RichEdit::m_useD2d = false;


EXTERN_C  IID IID_ITextServices = { // 8d33f740-cf58-11ce-a89d-00aa006cadc5
    0x8d33f740,
    0xcf58,
    0x11ce,
...
EXTERN_C  IID IID_ITextServices2 = { // B5DC90BA-F9A5-45DC-8C8A-784380C38769
	0xB5DC90BA,
	0xF9A5,
	0x45DC,
	{ 0x8C, 0x8A, 0x78, 0x43, 0x80, 0xC3, 0x87, 0x69 }
};


EXTERN_C  IID IID_ITextHost = { // c5bdd8d0-d26e-11ce-a89e-00aa006cadc5 
    0xc5bdd8d0,
    0xd26e,
    0x11ce,
...
public:
	...
	BOOL GetCanDrag();
	void SetDisableDrag(BOOL bDisableDrag);
...
private:
    ...
    ITextServices	*pserv;		    // pointer to Text Services object （Could Be ITextServices2）
    // Properties
	...
	unsigned    fDisableDrag        : 1;	// Whether disable/enable dragging
...
BOOL CTxtWinHost::Init(RichEdit *re, const CREATESTRUCT *pcs)
{
	...
	IID* pIID_ITS = (IID*)(VOID*)GetProcAddress(hmod,
		"IID_ITextServices");
	if(pIID_ITS != nullptr)
		memcpy((void*)&IID_ITextServices, pIID_ITS, sizeof(IID));
	IID* pIID_ITS2 = (IID*)(VOID*)GetProcAddress(hmod,
		"IID_ITextServices2");
	if(pIID_ITS2 != nullptr)
		memcpy((void*)&IID_ITextServices2, pIID_ITS2, sizeof(IID));
	IID* pIID_ITextHost = (IID*)(VOID*)GetProcAddress(hmod,
		"IID_ITextHost");
	if(pIID_ITextHost != nullptr)
		memcpy((void*)&IID_ITextHost, pIID_ITextHost, sizeof(IID));
	if (RichEdit::IsUseDirect2d())
	{
		ITextServices2 *pTxtServices2 = nullptr;
		hr = pUnk->QueryInterface(IID_ITextServices2, (void**)&pTxtServices2);
		if (FAILED(hr)) {
			throw std::exception("QueryInterface for ITextServices2 failure");
		}
		pserv = pTxtServices2;
	}
	else {
...
BOOL CTxtWinHost::GetCanDrag()
{
	return !fDisableDrag;
}

void CTxtWinHost::SetDisableDrag(BOOL bDisableDrag)
{
	fDisableDrag = bDisableDrag;
	pserv->OnTxPropertyBitsChange(TXTBIT_DISABLEDRAG,
		fDisableDrag ? TXTBIT_DISABLEDRAG : 0);
}
...
RichEdit::RichEdit() : 
	...
	m_lPrompttextStyle(DT_LEFT),
	m_rcPrompttextPadding(0,0,0,0),
...
void RichEdit::SetPasswordChar(WCHAR chPasswordChar)
{
	if (m_pTwh) m_pTwh->SetPasswordChar(chPasswordChar);
}
...
bool RichEdit::GetCanDrag()
{
	return !m_bDisableDrag;
}

void RichEdit::SetDisableDrag(bool bDisableDrag)
{
	m_bDisableDrag = bDisableDrag;
	if (m_pTwh) m_pTwh->SetDisableDrag(bDisableDrag);
}
...
double RichEdit::GetLineSpacing() const 
{
...
void RichEdit::SetLineSpacing(double spacing)
{
	static const unsigned kOneTwips = 1440;
	m_nLineSpacing = spacing;
	if (m_bInited)
	{
		PARAFORMAT2 pf2;
		...
		pf2.bLineSpacingRule = 4;   //取值0－5，具体含义看帮助
		LONG yPixPerInch = ::GetDeviceCaps(GetWindow()->GetPaintDC(), LOGPIXELSY);
		if (0 == yPixPerInch)
			yPixPerInch = 96;
		pf2.dyLineSpacing = DpiManager::GetInstance()->ScaleDouble(m_nLineSpacing) * (kOneTwips / yPixPerInch);
		SetParaFormat(pf2);
	}
}

void RichEdit::SetLimitMaxLines(int32_t nLimitMaxLines)
{
	m_nLimitMaxLines = nLimitMaxLines;
}
...
void RichEdit::PasteSpecial(UINT uClipFormat, DWORD dwAspect/* = 0*/, HMETAFILE hMF/* = 0*/)
{
	...
	if (uClipFormat == CF_TEXT)
	{
		CHARRANGE sel_pos;
		GetSel(sel_pos);
		if (sel_pos.cpMin != sel_pos.cpMax)
		{
			ReplaceSel(L"", TRUE);
		}
	}
...
// 				SendMessage(hwnd, WM_NOTIFY, EN_LINK, (LPARAM)&url);
				m_pWindow->SendNotify((Control*)this, kEventCustomLinkClick, 0, (LPARAM)&url);
			}
		}
		break;
	case EN_CHANGE:
...
ITextServices2* RichEdit::GetTextServices2()
{
	if (NULL == m_pTwh)
		return NULL;
	ITextServices* pTxtServices = m_pTwh->GetTextServices();
	ITextServices2* pTxtServices2 = nullptr;
	HRESULT hr = pTxtServices->QueryInterface(IID_ITextServices2,(void**)&pTxtServices2);
	if (SUCCEEDED(hr)) {
		return pTxtServices2;
	}
	else {
		return nullptr;
	}
...
CSize RichEdit::GetNaturalSize(LONG width, LONG height, bool bNeedDpiScale)
{
	if (width < 0) {
		width = 0;
	}
	...
	if (bNeedDpiScale)
	{
		lWidth = DpiManager::GetInstance()->ScaleIntEx(width);
		lHeight = DpiManager::GetInstance()->ScaleIntEx(height);
	}

	CSize sz(0,0);
	if (m_cbGetNaturalSize != nullptr && m_cbGetNaturalSize(lWidth, lHeight, sz))
		return sz;
...
CSize RichEdit::GetRichTextNaturalSize(LONG width, LONG height, bool bNeedDpiScale/* = true*/)
{
	if (width < 0) {
		width = 0;
	}
...
void RichEdit::SetTimer(UINT idTimer, UINT uTimeout)
{
	...
	TimerManager::GetInstance()->AddCancelableTimer(m_timeFlagMap[idTimer].GetWeakFlag(), ToWeakCallback(callback), uTimeout, TimerManager::REPEAT_FOREVER);
}
...
void RichEdit::HandleMessage(EventArgs& event)
{
	...
	if (event.Type == kEventMouseScrollWheel) {
		...
		Invalidate();	// 2021/05/27 解决滚动条不实时刷新的问题，暂时这么处理
	...
	if (event.Type == kEventMouseButtonDown || event.Type == kEventPointDown) {
		if (m_linkInfo.size() > 0)	{
			std::wstring strLink;
			CHARRANGE crLink;
			if (HittestCustomLink(event.ptMouse, strLink, crLink))
			{
				//::SetCursor(::LoadCursor(NULL, MAKEINTRESOURCE(IDC_HAND)));
				if (m_pWindow != NULL)
					m_pWindow->SendNotify((Control*)this, kEventCustomLinkClick, 0, (LPARAM)&strLink);
				return;
			}
		}
...
void RichEdit::OnSetCursor(EventArgs& event)
{
	...
	if (HittestCustomLink(event.ptMouse, strLink, crLink))
	{
		...
		if (m_cbCustomLinkMouseEnter != nullptr)
			m_cbCustomLinkMouseEnter(crLink);

		return;
	}


	if (m_cursorType == kCursorHandIbeam)
	{
...
void RichEdit::OnSetFocus(EventArgs& event)
{
	...
	SetImmStatus(TRUE);
	// add zys 2022/09/08 解决输入法显示位置不准确的问题
	if (m_iCaretPosX < 1 || m_iCaretPosY < 1)
	{
		if (m_imeUpdatePosFlag.HasUsed())
		{
			m_imeUpdatePosFlag.Cancel();
		}
		auto fn_cb = [this](){
			ImeUpdateCompositionPos();
		};
		TimerManager::GetInstance()->AddCancelableTimer(m_imeUpdatePosFlag.GetWeakFlag(), fn_cb, 200, 1);
		return;
	}
	ImeUpdateCompositionPos();
...
void  RichEdit::PasteTextNumberOnly()
{
	std::wstring strClipText;
	GetClipboardText(strClipText);
	if (!strClipText.empty()) {
...
void RichEdit::ImeUpdateCompositionPos()
{
	HWND hWnd = GetWindow()->GetHWND();
	if (hWnd == NULL)
		return;
...
void RichEdit::SetRichEditTextWithLimit( int nMaxLines)
{
	// 获取控件高度和每行高度
	int nControlHeight = GetPos().GetHeight();
	int nLineHeight = 20;
	// 计算限制行数的高度
	int nLimitHeight = nMaxLines * nLineHeight;
	// 获取文本行数
	int nLineCount = GetLineCount();
	// 计算文本总高度
	int nTotalHeight = nLineCount * nLineHeight;

	// 如果文本总高度超过限制高度，则截断最后一行并添加“...”
	if (nTotalHeight > nLimitHeight)
	{

		int nStartChar = LineIndex(nMaxLines);

		int nLineLength = LineLength(nLineCount - 1);
		int nEndChar   = LineIndex(nLineCount - 1) + nLineLength;

		std::wstring lineText = GetTextRange(nStartChar, nEndChar);

		SIZE szLastLine;
		::GetTextExtentPoint32(GetWindow()->GetPaintDC(), lineText.c_str(), lineText.length(), &szLastLine);
		szLastLine.cx = ui::DpiManager::GetInstance()->ScaleIntEx(szLastLine.cx);


		// 如果最后一行的宽度大于控件宽度，则截断最后一行
		if (szLastLine.cx > GetPos().GetWidth())
		{
			int nMaxStartLines = LineIndex(nMaxLines -1);
			int nMaxEndLines = nMaxStartLines + LineLength(nMaxLines - 1);

			for (int32_t nEndCharIndex = nMaxEndLines- 1 ; nEndCharIndex > nMaxStartLines; nEndCharIndex--)
			{
				std::wstring lineText = GetTextRange(nMaxStartLines, nEndCharIndex);

				SIZE nMaxEndLine;
				::GetTextExtentPoint32(GetWindow()->GetPaintDC(), lineText.c_str(), lineText.length(), &nMaxEndLine);
				nMaxEndLine.cx = ui::DpiManager::GetInstance()->ScaleIntEx(nMaxEndLine.cx);

				if (nMaxEndLine.cx <= GetPos().GetWidth())
				{
					SetSel(nEndCharIndex, nEndChar);
					ReplaceSel(_T("..."), true);
					break;
				}
			}
...
void RichEdit::Paint(IRenderContext* pRender, const UiRect& rcPaint)
{
...
#if 0
	if (m_nLimitMaxLines > 0)
	{	
		SetRichEditTextWithLimit(m_nLimitMaxLines);
	}
#endif
    ...
    if( m_pTwh ) {
		...
		HDC hDC = NULL;
		if (m_useD2d) {
			RenderContext_D3D *pD3DRenderContext = dynamic_cast<RenderContext_D3D*>(pRender);
			hDC = pD3DRenderContext->BeginGdiDraw(rc);
		}
		else {
			hDC = pRender->GetDC();
		}
		HRESULT hr = m_pTwh->GetTextServices()->TxDraw(
		...
		if (FAILED(hr))
		{
			throw std::exception("TxDraw failure");
		}
		if (m_bVScrollBarFixing) {
			LONG lWidth = rc.right - rc.left + m_pVerticalScrollBar->GetFixedWidth();
		...
		if (m_useD2d) {
			RenderContext_D3D *pD3DRenderContext = dynamic_cast<RenderContext_D3D*>(pRender);
			pD3DRenderContext->EndGdiDraw(hDC, rc, 0xFF000000 | RGB(GetRValue(cf.crTextColor), GetGValue(cf.crTextColor), GetBValue(cf.crTextColor)));
		}
#else
		if (m_useD2d) {
			RenderContext_D3D *pD3DRenderContext = dynamic_cast<RenderContext_D3D*>(pRender);
			//ID2D1RenderTarget *pTar = pD3DRenderContext->BeginD2DDraw();
			ID2D1RenderTarget *pTar = pD3DRenderContext->GetRender();
			ITextServices2 *pTxtService2 = GetTextServices2();
			HRESULT hr = pTxtService2->TxDrawD2D(pTar,
				(RECTL*)&rc,			// Bounding client rectangle
				(UiRect*)&rcPaint,		// Update rectangle
				0
			);
			if (FAILED(hr))
			{
				throw std::exception("TxDrawD2D failure");
			}
			//pD3DRenderContext->EndD2DDraw(rc);
			pTxtService2->Release();
		}
		else {
			HRESULT hr = m_pTwh->GetTextServices()->TxDraw(
				DVASPECT_CONTENT,  		// Draw Aspect
...
void RichEdit::SetAttribute(const std::wstring& strName, const std::wstring& strValue)
{
	...
	else if (strName == _T("prompttextalign")) {
		if (strName.find(_T("left")) != std::wstring::npos) {
			m_lPrompttextStyle &= ~(DT_CENTER | DT_RIGHT);
			m_lPrompttextStyle |= DT_LEFT;
		}
		if (strValue.find(_T("hcenter")) != std::wstring::npos) {
			m_lPrompttextStyle &= ~(DT_LEFT | DT_RIGHT);
			m_lPrompttextStyle |= DT_CENTER;
		}
		if (strValue.find(_T("right")) != std::wstring::npos) {
			m_lPrompttextStyle &= ~(DT_LEFT | DT_CENTER);
			m_lPrompttextStyle |= DT_RIGHT;
		}
		if (strValue.find(_T("top")) != std::wstring::npos) {
			m_lPrompttextStyle &= ~DT_BOTTOM;
			m_lPrompttextStyle |= DT_TOP;
		}
		if (strValue.find(_T("bottom")) != std::wstring::npos) {
			m_lPrompttextStyle &= ~DT_TOP;
			m_lPrompttextStyle |= DT_BOTTOM;
	...
	else if (strName == _T("prompttextpadding")) {
		UiRect rcPadding;
		LPTSTR pstr = NULL;
		m_rcPrompttextPadding.left = _tcstol(strValue.c_str(), &pstr, 10);  ASSERT(pstr);
		m_rcPrompttextPadding.top = _tcstol(pstr + 1, &pstr, 10);    ASSERT(pstr);
		m_rcPrompttextPadding.right = _tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);
		m_rcPrompttextPadding.bottom = _tcstol(pstr + 1, &pstr, 10); ASSERT(pstr);
	}
	else if (strName == _T("limitmaxlines"))SetLimitMaxLines(_ttoi(strValue.c_str()));
	else if (strName == _T("linespacing")) SetLineSpacing(_ttof(strValue.c_str()));
...
bool RichEdit::GetCaretPos(CPoint& pt)
{
	pt.x = m_iCaretPosX;
	pt.y = m_iCaretPosY;
	return true;
...
void RichEdit::PaintPromptText(IRenderContext* pRender)
{
	...
	UiRect rc;
	m_pTwh->GetControlRect(&rc);
	rc.left += m_rcPrompttextPadding.left;
	rc.right -= m_rcPrompttextPadding.right;
	rc.top += m_rcPrompttextPadding.top;
	rc.bottom -= m_rcPrompttextPadding.bottom;

	DWORD dwClrColor = GlobalManager::GetTextColor(m_sPromptColor);
	UINT dwStyle = m_lPrompttextStyle;
	dwStyle |= DT_NOCLIP;
...
void RichEdit::SetLinkColorBySel(long nStartChar, long nEndChar, const std::wstring &color, const std::wstring &linkInfo)
{
	if (!m_bRich || color.empty()) {
		ASSERT(FALSE);
		return;
	}
...
bool RichEdit::HittestCustomLink(CPoint pt, std::wstring& info, CHARRANGE& cr)
{
	bool bLink = false;
	info.clear();
	if (m_linkInfo.size() > 0) {
