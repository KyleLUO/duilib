...
public:
	 ...
	 * @return 无
	 */
	virtual void AttachSelect(const EventCallback& callback) { this->OnEvent[kEventSelect] += callback; }
...
void CheckBoxTemplate<InheritType>::SetAttribute(const std::wstring& strName, const std::wstring& strValue)
{
	...
	else if (strName == _T("checkimages"))
	{
		ParseCompositeImage(this, strValue, [this](int stateType, std::wstring AttributeValue)
		{
			if (stateType == 0)
			{
				SetStateImage(kControlStateNormal, AttributeValue);
			}
			else if (stateType == 1)
			{
				SetStateImage(kControlStateHot, AttributeValue);
			}
			else if (stateType == 2)
			{
				SetSelectedStateImage(kControlStateNormal, AttributeValue);
			}
			else if (stateType == 3)
			{
				SetSelectedStateImage(kControlStateDisabled, AttributeValue);
			}
			else if (stateType == 4)
			{
				SetStateImage(kControlStateDisabled, AttributeValue);
			}
		});
	}
	else if (strName == _T("checkselectedimages"))
	{
		ParseCompositeImage(this, strValue, [this](int stateType, std::wstring AttributeValue)
		{
			SetSelectedStateImage((ControlStateType)stateType, AttributeValue);
		});
	}
