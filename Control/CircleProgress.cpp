...
void CircleProgress::SetAttribute(const std::wstring& srName, const std::wstring& strValue)
{
	...
	else if (srName == _T("defaultbgcolor")) {
		LPCTSTR pValue = strValue.c_str();
		while (*pValue > _T('\0') && *pValue <= _T(' ')) pValue = ::CharNext(pValue);
		SetDefaultBackgroudColor(pValue);
	}
...
void CircleProgress::PaintStatusImage(IRenderContext* pRender)
{
	...
	if (m_bCircular)
	{
		...
		if (m_dwGradientColor == 0)
		{
			...
			Gdiplus::SolidBrush fgBrush(m_dwBackgroundColor);
			...
			outer.Inflate(-2, -2);
			graphics.DrawEllipse(&fgPen, outer);
			outer.Inflate(-2, -2);

			if (m_dwDefaultBkgroundColor > 0)
			{
				Gdiplus::SolidBrush fgDefaultBrush(m_dwDefaultBkgroundColor);
				graphics.FillPie(&fgDefaultBrush, outer, 0, 360);
			}

			graphics.FillPie(&fgBrush, outer, 270, direction * 360 * (m_nValue - m_nMin) / (m_nMax - m_nMin));
		}
		else
		{
			...
			lgbrush.SetBlend(factors, positions,3);
...
void CircleProgress::SetDefaultBackgroudColor(const std::wstring& strColor)
{
	m_dwDefaultBkgroundColor = GlobalManager::GetTextColor(strColor);
	ASSERT(m_dwDefaultBkgroundColor != 0);
	Invalidate();
}
