...
Shadow::Shadow(const wchar_t* bk_path) :
m_rcCurShadowCorner(12, 12, 12, 12),
m_rcDefaultShadowCorner(12, 12, 12, 12),
	m_szRoundCorner(2, 2),
...
Box*Shadow::AttachShadow(Box* pRoot)
{
	...
	{
		pRoot->SetBorderRound(m_szRoundCorner);
	}
...
void Shadow::MaximizedOrRestored(bool isMaximized)
{
	...
	if (isMaximized && m_pRoot) {
		...
		{
			Control* control = m_pRoot->GetItemAt(0);
			control->SetBorderRound(m_szRoundCorner);
		}
	}
}
...
void Shadow::SetRoundCorner(int cx, int cy)
{
