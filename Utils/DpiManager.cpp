...
void DpiManager::SetScale(UINT uDPI)
{
	if (m_bAdaptDPI)
	{
		...
		if (m_nScaleFactor <= 115)
		{
			m_nScaleFactor = 100;
		}
		else if (m_nScaleFactor > 115 && m_nScaleFactor < 135)  
		{
			m_nScaleFactor = 125;
		}
		else if (m_nScaleFactor >= 135)
		{
			m_nScaleFactor = 150;
		}
	}
...
int DpiManager::ScaleIntEx(int nValue)
{
	if (!m_bAdaptDPI || m_nScaleFactor == 100)
		return nValue;

	int result = MulDiv(nValue, m_nScaleFactor, 100);
	return result;
}

double DpiManager::ScaleDouble(double dValue)
{
	if (!m_bAdaptDPI || m_nScaleFactor == 100)
		return dValue;

	double result = dValue * m_nScaleFactor / 100;
	return result;
}
