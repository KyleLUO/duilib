...
CSize Layout::ArrangeChild(const std::vector<Control*>& items, UiRect rc)
{
	...
	for (auto it = items.begin(); it != items.end(); it++) 
	{
		...
		if (pControl->GetWeakFlag().expired())
		{
			continue;
		}
...
void ScrollableBox::SetAttribute(const std::wstring& pstrName, const std::wstring& pstrValue)
{
	...
	else if (pstrName == _T("scrollcomplete")) SetScrollComplete(pstrValue == _T("true"));
...
void ScrollableBox::SetScrollPos(CSize szPos)
{
	...
	if( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsValid() ) {
		...
		m_lasthScrollPos = iLastScrollPos;
...
void ScrollableBox::LineToPosY(int y, bool withAnimation)
{
	CSize scrollPos = GetScrollPos();
	if (!withAnimation)
	{
	...
	{
		int start_cy = scrollPos.cy;
		int end_cy = y;

		m_scrollAnimation.SetStartValue(start_cy);
		m_scrollAnimation.SetEndValue(end_cy);
		...
		if (m_pWindow) {
			m_pWindow->SendNotify(this, kEventScrollStart);
		}
		m_scrollAnimation.SetCompleteCallback(nbase::Bind(&ScrollableBox::OnScrollComplete, this));
...
void ScrollableBox::OnScrollComplete()
{
	if (m_pWindow != NULL)	{
		m_pWindow->SendNotify(this, kEventScrollEnd);
		if (IsScrollComplete())
		{
			POINT pt = { 0 };
			::GetCursorPos(&pt);
			::ScreenToClient(m_pWindow->GetHWND(), &pt);
			::SendMessage(m_pWindow->GetHWND(), WM_MOUSEMOVE, 0, (LPARAM)MAKEWORD(pt.x, pt.y));
		}
	}
...
void ScrollableBox::ProcessHScrollBar(UiRect rc, int cxRequired)
{
	...
	m_pHorizontalScrollBar->SetScrollPos(m_lasthScrollPos);
}
...
bool ScrollableBox::IsScrollComplete() const
{
	return m_bScrollComplete;
}

void ScrollableBox::SetScrollComplete(bool bScrollComplete)
{
	m_bScrollComplete = bScrollComplete;
}
