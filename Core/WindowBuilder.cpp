			...
			for( CMarkupNode node = root.GetChild() ; node.IsValid(); node = node.GetSibling() ) {
				...
				if( strClass == _T("Image") ) {
					...
					if( !strFontName.empty() ) {
						GlobalManager::AddFont(strFontId, strFontName, size, bold, underline, italic, isDefault, escapement, orientation,strikeout);
					}
				}
...
Control* WindowBuilder::CreateControlByClass(const std::wstring& strControlClass)
{
	...
	case 10:
		...
		else if (strControlClass == DUI_CTR_OPTIONVBOX)   pControl = new OptionVBox;
		else if (strControlClass == DUI_CTR_BUTTONHBOX)         pControl = new ButtonHBox;
		else if (strControlClass == DUI_CTR_BUTTONVBOX)         pControl = new ButtonVBox;
		break;
	case 11:
