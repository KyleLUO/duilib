...
//////////////////////////////////////////////////////////////////////////
///
Window::Window(const wchar_t *bk_path) :
	m_hWnd(nullptr),
	m_pRoot(nullptr),
	OnEvent(),
...
Window::~Window()
{
	...
	if (m_pToolTipCtrl)
	{
		::DestroyWindow(m_pToolTipCtrl->GetHWND());
	}

	if (m_pToolTipEmoji)
	{
		::DestroyWindow(m_pToolTipEmoji->GetHWND());
	}
#endif
...
HWND Window::Subclass(HWND hWnd)
{
	...
	ASSERT(m_hWnd == NULL);
	m_OldWndProc = MySubclassWindow(hWnd, __WndProc);
...
void Window::Unsubclass()
{
	...
	MySubclassWindow(m_hWnd, m_OldWndProc);
	m_OldWndProc = ::DefWindowProc;
	m_bSubclassed = false;
}
...
void PrintWndLog(HWND parent_hwnd,HWND hWnd,const std::wstring &func_name)
{
	wchar_t parent_name[256] = { 0 };
	::GetClassName(parent_hwnd, parent_name, sizeof(parent_name));

	wchar_t name[256] = { 0 };
	::GetClassName(hWnd, name, sizeof(name));

	KY_QLOG_ERR(L"ClassName={0} parentName={1} func_name={2}") << name << parent_name<< func_name;
}
...
void Window::Close(UINT nRet)
{
	if (m_bFakeModal)
	{
		...
		PrintWndLog(parent_hwnd, m_hWnd,L"Close");
	}
	ASSERT(::IsWindow(m_hWnd));
	if (!::IsWindow(m_hWnd)) return;
	...
	if (m_pRoot && IsWindowsVistaOrGreater()) {
		...
		TimerManager::GetInstance()->AddCancelableTimer(m_closeFlag.GetWeakFlag(), closeCallback, 50, 1);
	}
	else {
...
void Window::ShowModalFake(HWND parent_hwnd)
{
	...
	PrintWndLog(parent_hwnd, m_hWnd,L"ShowModalFake");
}
...
void Window::CenterWindow(HWND hwndFrom, bool wndCenter/* = false*/)
{
	...
	if (hWndCenter!=NULL)
		hWnd=hWndCenter;
	if (hWndCenter == NULL && hwndFrom != NULL)
	{
		hWnd = hwndFrom;
		if (wndCenter)
		{
...
void Window::AttachMouseLeave(const EventCallback& callback)
{
...
void Window::Init(HWND hWnd)
{
	...
	m_renderContext = GlobalManager::CreateRenderContext(hWnd);
...
void Window::SetWindowResourcePath(const std::wstring& strPath)
{
	...
	m_renderContext->SetResourceRoot(m_strWindowResourcePath); //设置本地资源根目录
}
...
void Window::SetAlphaFixCorner(UiRect& rc, bool dpi/* = true*/)
{
	if (dpi)
	{
...
void Window::EnableWindowWarp(bool enable)
{
	::EnableWindow(m_hWnd,enable);

	HWND hWndParent = GetWindowOwner(m_hWnd);
	PrintWndLog(hWndParent, m_hWnd,L"EnableWindowWarp");
}
...
bool Window::PreMessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& /*lRes*/)
{
	...
	{
		if (m_pFocus != NULL) {
			m_pFocus->HandleMessageTemplate(EventType::kEventSystemKey, 0, 0, wParam);
		}
	}
	break;
	}
...
void Window::SetMinInfo(int cx, int cy, bool bNeedDpiScale, bool bContainShadow)
{
	if (bNeedDpiScale)
	{
		DpiManager::GetInstance()->ScaleInt(cx);
...
bool Window::HandleMouseEnterLeave(const POINT &pt, WPARAM wParam, LPARAM lParam)
{
	...
	if (m_pNewHover != m_pEventHover && m_pNewHover != NULL) {
		...
		if (m_pNewHover)
		{
			std::wstring sToolTipMode = m_pNewHover->GetToolTipMode();
			if (sToolTipMode == L"none" || sToolTipMode == L"top" || sToolTipMode == L"right" || sToolTipMode == L"bottom" || sToolTipMode == L"left")
			{
				CreatTooltipCtrlWindow();
			}
			else if (sToolTipMode == L"emoji")
			{
				CreatTooltipEmojiWindow();
			}
			else 
			{
				CreatTooltipWindow();
...
void Window::CreatTooltipWindow()
{
	// Create tooltip information
	if (m_pNewHover == nullptr)
		return;
	std::wstring sToolTip = m_pNewHover->GetToolTipText();
	if (sToolTip.empty())
		return;
	...
	ui::UiRect pos = m_pNewHover->GetPos();
	std::wstring sToolTipMode = m_pNewHover->GetToolTipMode();
	if (sToolTipMode == L"center")
	{
		//int offset_x = DpiManager::GetInstance()->ScaleIntEx(10);
		//int offset_y = DpiManager::GetInstance()->ScaleIntEx(20);
		//pos.Deflate(offset_x, offset_y);
		m_ToolTip.uFlags = TTF_CENTERTIP;
	}
	...
	::SendMessage(m_hwndTooltip, TTM_SETMAXTIPWIDTH, 0, m_pNewHover->GetToolTipWidth());
	::SendMessage(m_hwndTooltip, TTM_SETTOOLINFO, 0, (LPARAM)&m_ToolTip);
	::SendMessage(m_hwndTooltip, TTM_TRACKACTIVATE, TRUE, (LPARAM)&m_ToolTip);
}


void Window::CreatTooltipCtrlWindow()
{
	if (m_pNewHover == nullptr)
		return;
	std::wstring sToolTip = m_pNewHover->GetToolTipText();
	...
	//ui::UiRect ctrl_pos = m_pNewHover->GetPos();
	ui::UiRect ctrl_pos = m_pNewHover->GetPosWithScrollOffset();

	std::wstring sToolTipMode = m_pNewHover->GetToolTipMode();
	int  distance = m_pNewHover->GetToolTipDistance();
	int show_type = -1;
	if (sToolTipMode == L"left")
		show_type = 0;
	else if (sToolTipMode == L"top")
		show_type = 1;
	else if (sToolTipMode == L"right")
		show_type = 2;
	else if (sToolTipMode == L"bottom")
		show_type = 3;

	if (!m_pToolTipCtrl)
	{
		m_pToolTipCtrl = new ToolTipCtrl;
		m_pToolTipCtrl->CreateWnd(m_hWnd);
	}
	m_pToolTipCtrl->ShowWindow(true, false);
	m_pToolTipCtrl->SetDistance(distance);
	m_pToolTipCtrl->SetShowMode(show_type);
	m_pToolTipCtrl->SetTipText(sToolTip);
	m_pToolTipCtrl->SetShowPos(ctrl_pos);

}

void Window::HideTooltipCtrlWindow()
{
	if (m_pToolTipCtrl && IsWindowVisible(m_pToolTipCtrl->GetHWND()))
	{
		m_pToolTipCtrl->ShowWindow(false, false);
	}
}


void Window::CreatTooltipEmojiWindow()
{
	if (m_pNewHover == nullptr)
		return;
	std::wstring sToolTip = m_pNewHover->GetToolTipText();
	...
	if (!m_pToolTipEmoji)
	{
		m_pToolTipEmoji = new ToolTipEmoji;
		m_pToolTipEmoji->CreateWnd(m_hWnd);
	}
	m_pToolTipEmoji->ShowWindow(true, false);
	m_pToolTipEmoji->SetDistance(distance);
	m_pToolTipEmoji->SetShowMode(show_type);
	m_pToolTipEmoji->SetTipText(sToolTip);
	m_pToolTipEmoji->SetShowPos(ctrl_pos);
}

void Window::HideTooltipEmojiWindow()
{
	if (m_pToolTipEmoji && IsWindowVisible(m_pToolTipEmoji->GetHWND()))
	{
		m_pToolTipEmoji->ShowWindow(false, false);
	}
}
...
void Window::Paint()
{
	...
	m_renderContext->BeginPaint(&rcPaint);
	...
	if (m_bIsLayeredWindow) {
		...
		BOOL ret = ::UpdateLayeredWindow(m_hWnd, NULL, &pt, &szWindow, m_renderContext->GetDC(), &ptSrc, 0, &bf, ULW_ALPHA);
		if (ret == FALSE) {
			DWORD err = GetLastError();
			RaiseException(err, 0, 0, NULL);
		}
	}
	else {
		BOOL ret = ::BitBlt(ps.hdc, rcPaint.left, rcPaint.top, rcPaint.GetWidth(),
