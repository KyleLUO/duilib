...
typedef struct tagTFontInfo
{
	...
	bool bItalic;
	TEXTMETRIC tm;
	Microsoft::WRL::ComPtr<IDWriteTextFormat> dWriteFmt;
	Microsoft::WRL::ComPtr<IDWriteInlineObject> dWriteTrimObj;
	Microsoft::WRL::ComPtr<IDWriteFontFace> dWriteFontFace;
} TFontInfo;
...
{
public:
    Window(const wchar_t *bk_path=L"");
	~Window();
	 ...
	 * @return 无
	 */
	virtual void ShowModalFake(HWND parent_hwnd);

	/**
	 * @brief 居中窗口，支持扩展屏幕
	 * @return 无
	 */
	void CenterWindow(HWND hwndFrom = nullptr, bool wndCenter = false);
...
public:
	...
	void RemoveOptionGroup(const std::wstring& strGroupName);
	 ...
	 * @return 无
	 */
	void SetAlphaFixCorner(UiRect& rc,bool dpi = true);

	void EnableWindowWarp(bool enable);
	UINT ShowModal();
	/**
	 ...
	 */
	// 
	void SetMinInfo(int cx, int cy, bool bNeedDpiScale = true, bool bContainShadow = false);
	...
	void CreatTooltipWindow();

	void CreatTooltipCtrlWindow();
	void HideTooltipCtrlWindow();

	void CreatTooltipEmojiWindow();
	void HideTooltipEmojiWindow();
	/**
...
protected:
	...
	ui::ToolTipCtrl* m_pToolTipCtrl = nullptr;

	ui::ToolTipEmoji* m_pToolTipEmoji = nullptr;
};
