...
public:
	 ...
	 * @param[in] theme 主题目录名，默认为 themes\\xml
	 * @param[in] language 使用语言，默认为 lang\\zh_CN
	 * @return 无
	 */
	static void Startup(const std::wstring& strResourcePath, const CreateControlCallback& callback, bool bAdaptDpi, const std::wstring& theme = L"themes\\xml", const std::wstring& language = L"lang\\zh_CN");
	...
	/**
	* @brief 获取APP资产所在目录
	* @return 返回当前APP资产所在目录
	*/
	static std::wstring GetResourceAssetsPath();
	...
	* @return 无
	*/
	static void SetResourceAssetsPath(const std::wstring& strPath);
	...
	// 创建控件，因为在引用duilib库的文件调用new，会提示重复定义
	static Control* CreateOptionBox();

	/**
	 * @brief 创建全局的绘制上下文区域
	 * @return 返回绘制区域对象
	 */
	static std::unique_ptr<IRenderContext> CreateRenderContext(HWND hwnd=NULL);
	/**
	* @brief 创建全局的绘制上下文区域
	* @return 返回绘制区域对象
	*/
	static std::unique_ptr<IRenderContext> CreateRenderContext(ui::IRenderContext *pSourceRender=nullptr);
	 ...
	 * @return 返回画刷对象
	 */
	static std::unique_ptr<IBrush> CreateBrush(DWORD color,ui::IRenderContext* ctx = nullptr);
	 ...
	 * @return 返回绘制路径对象
	 */
	static std::unique_ptr<IPath> CreatePath(ui::IRenderContext* ctx = nullptr);
	 ...
	 */
	static HFONT AddFont(const std::wstring& strFontId, const std::wstring& strFontName, int nSize, bool bBold, bool bUnderline, bool bItalic, bool bDefault, int nEscapement, int nOrientation, bool strikeout = false);
...
private:
	...
	static std::wstring m_pStrResourceAssetsPath; //全局资产路径
