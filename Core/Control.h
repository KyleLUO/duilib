...
UILIB_API void ParseCompositeImage(Control *control, const std::wstring& strValue, std::function<void(int, std::wstring)> OnStateImage);
...
public:
	...
	void SetBkColorAlpha(int alpha);

	/*
	* 渐变属性设置
	*/
	bool GetBKGradient();
	void SetBKGradient(const bool bGradient);
	void SetBkGradientColor(const std::wstring& strColor, const std::wstring& strColor1);
	void SetBkGradientColor1(const std::wstring& strColor);
	void SetBkGradientColor2(const std::wstring& strColor);
	void SetBKGradientMode(const int mode);
	...
	void SetBorderRound2(float fRound);
	...
	void  SetToolTipMode(const std::wstring& mode);

	std::wstring  GetToolTipMode();
	void SetToolTipDistance(int distance);
	int  GetToolTipDistance();
	/**
	...
	* @return 返回绘制上下文对象
	*/
	IRenderContext* GetRenderContext(ui::IRenderContext* pSourceRender=nullptr);
	...
	*/
	 ...
	 * @return 无
	 */
	virtual void SetRenderOffsetX(int renderOffsetX);
...
protected:
	...
	void BeginPaint(IRenderContext* pRender);
	...
	void EndPaint(IRenderContext* pRender);
...
protected:
	...
	Gdiplus::DashStyle m_nBorderStyle = Gdiplus::DashStyleSolid;		//边框样式，换椭圆角的边框时用到
	CursorType m_cursorType;	//影响控件的鼠标形状
	ControlStateType m_uButtonState;
	std::wstring m_sToolTipText;
	std::wstring m_sToolTipTextId;
	std::wstring m_sToolTipMode =L"";
	int    m_sToolTipDistance = 0;
	std::wstring m_sUserData;
	std::wstring m_strBkColor;
	int m_nBkColorAlpha = 0xff000000;
	bool m_bBKGradient;	//启用背景渐变
	std::wstring m_strBkGradientColor1;	//背景渐变色1
	std::wstring m_strBkGradientColor2;	//背景渐变色2
	int m_nBKGradientMode;	//背景渐变模式：0：横向，1：纵向
	...
	bool m_UseGdiPlus = false;	//使用gdi+绘制背景
	int m_FillType = 0;	 //背景图片缩放：1填充模式
	bool m_isRoundBkImage = false;	// 是否绘制圆图
	bool m_bkSolidCircle = false;
	int  m_bRoundStyle = 0;
	float m_fBorderRound2 = 0;
};
