...
std::wstring GlobalManager::m_pStrResourceAssetsPath;
...
std::wstring GlobalManager::m_strDefaultDisabledColor = L"color_ffbfbfbf";
std::wstring GlobalManager::m_strDefaultFontColor = L"color_ff0b0d0c";
...
{
	try {
		m_renderFactory = std::make_unique<RenderFactory_D3D>();
		//m_renderFactory = std::make_unique<RenderFactory_GdiPlus>();
		RichEdit::SetUseDirect2d();
	}
	catch (std::exception& exc) {
		m_renderFactory = std::make_unique<RenderFactory_GdiPlus>();
	}
	GlobalManager::SetResourceAssetsPath(strResourcePath);
...
std::wstring GlobalManager::GetResourceAssetsPath()
{
	return m_pStrResourceAssetsPath;
}
...
void GlobalManager::SetResourceAssetsPath(const std::wstring& strPath)
{
	m_pStrResourceAssetsPath = strPath;
	if (m_pStrResourceAssetsPath.empty()) return;
	TCHAR cEnd = m_pStrResourceAssetsPath.at(m_pStrResourceAssetsPath.length() - 1);
	if (cEnd != _T('\\') && cEnd != _T('/')) m_pStrResourceAssetsPath += _T('\\');
}
...
Control *GlobalManager::CreateLabel()
{
	return new Label();
}

Control* GlobalManager::CreateOptionBox()
{
	return new OptionBox;
}

std::unique_ptr<ui::IRenderContext> GlobalManager::CreateRenderContext(HWND hwnd)
{
	std::unique_ptr<ui::IRenderContext> p;
	p.reset(m_renderFactory->CreateRenderContext(hwnd));
	return p;
}

std::unique_ptr<IRenderContext> GlobalManager::CreateRenderContext(ui::IRenderContext *pSourceRender /*= nullptr*/)
{
	std::unique_ptr<ui::IRenderContext> p;
	p.reset(m_renderFactory->CreateRenderContext(pSourceRender));
	return p;
}
...
std::unique_ptr<ui::IBrush> GlobalManager::CreateBrush(DWORD color, ui::IRenderContext* ctx /*= nullptr*/)
{
	std::unique_ptr<ui::IBrush> p;
	p.reset(m_renderFactory->CreateBrush(color, ctx));
	return p;
}
...
std::unique_ptr<ui::IPath> GlobalManager::CreatePath(ui::IRenderContext* ctx /*= nullptr*/)
{
	std::unique_ptr<ui::IPath> p;
	p.reset(m_renderFactory->CreatePath(ctx));
	return p;
}
...
HFONT GlobalManager::AddFont(const std::wstring& strFontId, const std::wstring& strFontName, int nSize, bool bBold, bool bUnderline, bool bItalic, bool bDefault, int nEscapement, int nOrientation, bool strikeout/* = false*/)
{
	if (nEscapement != 0)
	{
...
TFontInfo* GlobalManager::GetTFontInfo(const std::wstring& strFontId)
{
	...
	if (iter == m_mCustomFonts.end())
	{
		ASSERT(!m_sDefaultFontId.empty());
		strFindId = m_sDefaultFontId;
		iter = m_mCustomFonts.find(strFindId);
	}
