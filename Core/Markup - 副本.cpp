...
//
//
std::map<std::wstring, std::pair<PVOID, std::size_t>> CMarkup::s_xmlMapping;
...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
	...
	LPVOID pMappedFile = NULL;
	DWORD dwSize = 0;
	auto iter = s_xmlMapping.find(sFile);
	if (s_xmlMapping.end() != iter)
	{
		pMappedFile = (iter->second).first;
		dwSize = (iter->second).second;
	}
	else {
		...
		// 创建文件的内存文件映射
		HANDLE hMapFile = CreateFileMapping(hFile, NULL, PAGE_READONLY, 0, dwSize, NULL);
		CloseHandle(hFile);
		if (hMapFile == NULL)
		{
		...
		pMappedFile = MapViewOfFile(hMapFile, FILE_MAP_READ, 0, 0, 0);
		...
		s_xmlMapping[sFile] = std::make_pair(pMappedFile, dwSize);
	}
    
    bool ret = LoadFromMem((BYTE*)pMappedFile, dwSize, encoding);
    return ret;
}
