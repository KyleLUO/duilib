...
namespace ui 
{
	...
	class RenderNotImplemented : public std::exception {
	public:
		RenderNotImplemented(const std::string& message = "Not implemented")
			: m_message(message) {
		}
...
class UILIB_API IClip2 : public IClip
{
public:
...
public:
	virtual void CreateClip2(HDC hDC, ID2D1RenderTarget *pRender, UiRect rc) = 0;
	virtual void CreateRoundClip2(HDC hDC, ID2D1RenderTarget *pRender, UiRect rc, int width, int height) = 0;
	virtual void ClearClip2(ID2D1RenderTarget *pRender) = 0;
};
...
public:
	...
	virtual DWORD GetColor()const { return color_; };
...
public:
	...
	virtual DWORD GetColor()const { return color_; };
	virtual const IBitmap* GetBitmap()const { return bitmap_.get(); };
...
public:
	...
	virtual HRESULT BitBlt(int x, int y, int cx, int cy, IRenderContext *srcCtx, int xSrc = 0, int yScr = 0, DWORD rop = SRCCOPY) = 0;
	virtual bool AlphaBlend(int xDest, int yDest, int widthDest, int heightDest, IRenderContext *srcCtx, int xSrc, int yScr, int widthSrc, int heightSrc, BYTE uFade = 255) = 0;
	...
	virtual void DrawGradientColor(const UiRect& rc, const std::wstring& colorStr, const std::wstring& colorStr1, const int mode) = 0;
	...
	virtual void DrawHighlightText(const UiRect& rc, const std::wstring& strText, DWORD dwTextColor,
		...
		const std::wstring& strFontId, UINT uStyle, BYTE uFade = 255, bool bLineLimit = false) = 0;
	
	virtual void DrawHtmlText(const UiRect& rc_item, const std::wstring& strText, DWORD dwTextColor, const std::wstring& strFontId, UINT uStyle, RECT* prcLinks) = 0;
	...
	//新增设置windows 资源根路径
	virtual void SetResourceRoot(const std::wstring &path) = 0;
	//判断是否为dx模式
	virtual bool UseDirectX() = 0;
	virtual ID2D1RenderTarget* GetRender() = 0;

	//矩阵变换相关
	virtual void SetTransform() = 0;
	virtual void ResetTransform() = 0;

	virtual void TranslateTransform(int dx, int dy) = 0;
	virtual void RotateTransform(float angle) = 0;

	//GDI+ 无任何操作，为了兼容DX模式
	virtual void BeginPaint(const UiRect* rcPaint) = 0;
...
{
public:
	virtual ui::IPen* CreatePen(DWORD color, int width = 1, ui::IRenderContext* ctx = nullptr) = 0;
	virtual ui::IBrush* CreateBrush(DWORD color, ui::IRenderContext* ctx=nullptr) = 0;
	virtual ui::IBrush* CreateBrush(HBITMAP bitmap, ui::IRenderContext* ctx = nullptr) = 0;
	virtual ui::IMatrix* CreateMatrix() = 0;
	virtual ui::IPath* CreatePath(ui::IRenderContext* ctx = nullptr) = 0;
	virtual ui::IBitmap* CreateBitmap(ui::IRenderContext* ctx = nullptr) = 0;
	virtual ui::IRenderContext* CreateRenderContext(HWND wnd=NULL) = 0;
	virtual ui::IRenderContext* CreateRenderContext(ui::IRenderContext *pSourceRender) = 0;
};
